...
/world/proc/update_status()
	...
	if(ticker)
		if(master_mode)
			hub_info += "<br><b>Mode</b>: [master_mode]"
	else
		hub_info += "<br><b>STARTING</b>"
	...
	if(config.respawn)
		hub_info += "<br><b>Respawn Rate</b>: [resp_rate] [time_measure]"
	else
		hub_info += "<br><b>Respawn</b>: Disabled"

	if (config)
		hub_info += "<br><b>Restart Vote</b>:"

	hub_info += config.allow_vote_restart ? "Enabled" : "Disabled"

	if (src.status != hub_info)
		src.status = hub_info
